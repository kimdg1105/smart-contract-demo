...
contract MintTokenContract is ERC721Enumerable {
    ...
    // constructor
    constructor() ERC721("Dongguri", "DGR") {}
    ...
    function getTokens(address _tokenOwner)
        public
        view
        returns (DGRTokenData[] memory)
    {
        uint256 balanceLength = balanceOf(_tokenOwner);
        require(balanceLength > 0, "No tokens");

        DGRTokenData[] memory tokenDataArray = new DGRTokenData[](
            balanceLength
        );
        ...
        for (uint256 i = 0; i < balanceLength; i++) {
            ...
            tokenDataArray[i] = DGRTokenData(tokenId, tokenType, tokenPrice);
        }
